...
1. **html-to-image (or html2canvas) for DOM → PNG.** Why: React Flow nodes are HTML elements,
   ...
   than `html2canvas`. Falls back to `dom-to-image-more` if CORS issues arise.
